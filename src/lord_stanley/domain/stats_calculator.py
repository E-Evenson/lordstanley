...
def calculate_league_standings(
    ...
    win_counts = (
        completed_cup_games_with_owners["winner_owner"]
        .value_counts()
        .rename_axis("owner")
    )
...
def calculate_team_stats(
    ...
    win_counts = (
        completed_cup_games_with_owners["winner_abbrev"]
        .value_counts()
        .rename_axis("team_abbrev")
    )
    games_played = win_counts.add(
...
def calculate_cumulative_owner_stats(
    ...
    # groupby().transform() raises on an empty df
    if owners_cumulative_stats.empty:
        logger.debug("No completed cup games, skipping cumulative fill.")
